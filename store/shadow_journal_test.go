...
func TestShadowJournalAppendAndLatest(t *testing.T) {
	...
	if err := s.Append(entry); err != nil { t.Fatal(err) }
	if entry.ID == 0 { t.Fatal("expected persisted journal ID") }

	got, err := s.Latest("shadow-1", 10)
	if err != nil { t.Fatal(err) }
	if len(got) != 1 { t.Fatalf("expected 1 journal row, got %d", len(got)) }
